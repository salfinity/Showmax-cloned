import { NextPageContext } from "next";
import { getSession, useSession } from "next-auth/react";

import useCurrentUser from "@/hooks/useCurrentUser";
import { useRouter } from "next/router";
import { useCallback } from "react";

const images = [
  '/images/default-blue.png',
  '/images/default-green.png',
  '/images/default-yellow.png',
  '/images/default-black.png'
]

interface UserCardProps {
  name: string;
}

export async function getServerSideProps(context: NextPageContext) {
  const session = await getSession(context);

  if(!session) {
   return {
     redirect: {
      destination: '/auth',
      permanent: false,
     }
   }
  }

  return {
   props: {}
  }
} 

const UserCard: React.FC<UserCardProps> = ({ name }) => {
  const imgSrc = images[Math.floor(Math.random() * 4)];

  return (
    <div className="group flex-col w-44 mx-auto">
        <div className="w-44 h-44 rounded-full flex items-center justify-center border-2 border-transparent group-hover:cursor-pointer group-hover:border-white overflow-hidden">
          <img draggable={false} className="w-max h-max object-contain" src={imgSrc} alt="" />
        </div>
      <div className="mt-4 text-gray-400 text-2xl text-center group-hover:text-white">{name}</div>
   </div>
  );
}

const App = () => {
  const router = useRouter();
  const { data: currentUser } = useCurrentUser();

  const selectProfile = useCallback(() => {
    router.push('/');
  }, [router]);

 return (
  <div className="flex items-center h-full w-full justify-center flex-col min-h-screen">
    <nav className="px-12 py-5 flex justify-center relative">
     <img src="/images/logo.png" className="h-40" alt="Logo" />
     </nav>
     <div className="flex flex-col justify-center">
      <h1 className="text-white text-3xl md:text-6xl font-bold">Who's watching Showmax?</h1>
      <div className="flex items-center justify-center gap-8 mt-10">
       <div onClick={() => selectProfile()}>
          <UserCard name={currentUser?.name} />
        </div>
       </div>
       <div className="flex flex-row justify-center gap-10 mt-10 cursor-pointer">
         <span className="hover:bg-zinc-300 border border-white text-white hover:text-black font-bold py-2 px-4 rounded-full">
           Manage prifiles
          </span>
          <p className="flex flex-row hover:bg-zinc-300 border border-white text-white hover:text-black font-bold py-2 px-4 rounded-full">
           Manage prifiles
          </p>
         </div>
      </div>
    </div>
 )
};

export default App;